...
import { Flame, Clock, Trash2, ChevronDown, Link as LinkIcon, MoreVertical, ArrowUp, ArrowDown, Search, FileEdit, CheckCircle2, Circle, Video, GripVertical } from 'lucide-react';
...
const STATUS_LABELS_SHORT = {
  WISHLIST: '清单', APPLIED: '已投', ASSESSMENT: '测评', INTERVIEW: '面试', OFFER: 'Offer', REJECTED: '拒绝',
};
...
export function ApplicationCard({ application, onStatusChange, onDelete, onUpdateNotes, onUpdatePrepared, onEdit, onUpdateRejectReason }) {
  ...
  useEffect(() => {
    ...
    function updateCountdown() {
      ...
      if (days > 0) {
        setCountdown(`距离面试：${days}天 ${hours}小时`);
      } else if (hours > 0) {
        setCountdown(`距离面试：${hours}小时 ${minutes}分`);
      } else {
        setCountdown(`距离面试：${minutes}分钟`);
      }
    }
  ...
  return (
    ...
    >
      <div className="absolute top-2 left-2 z-10 opacity-0 group-hover:opacity-100 transition-all duration-200">
      ...
      <div className="absolute top-2 right-2 flex items-center gap-1 z-10 opacity-0 group-hover:opacity-100 transition-all duration-200">
        {isInterview && application.meetingLink && (
          <a href={application.meetingLink} onClick={handleMeetingLinkClick} className="p-1.5 rounded-lg text-primary hover:bg-primary-light transition-all duration-200" title="进入面试">
            <Video size={14} />
          </a>
        )}
        <a href={application.applyLink || '#'} onClick={handleLinkClick} className={`p-1.5 rounded-lg transition-all duration-200 ${application.applyLink ? 'text-gray-300 hover:text-primary hover:bg-primary-light' : 'text-gray-200 cursor-not-allowed'}`} title={application.applyLink ? '打开投递链接' : '暂无链接'}>
          <LinkIcon size={14} />
        </a>
        <div className="relative" ref={menuRef}>
          <button onClick={() => setShowMenu(!showMenu)} className="p-1.5 rounded-lg text-gray-300 hover:text-slate-600 hover:bg-gray-100 transition-all duration-200" title="更多操作">
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-1.5 mb-3 flex-wrap">
          ...
          {source && (
            <span className={`px-2 py-0.5 rounded-full text-xs font-medium border ${sourceColor}`}>{source}</span>
          )}
          {tagColor && (
            <span className={`px-2 py-0.5 rounded-full text-xs font-medium border ${tagColor}`}>{application.resumeTag}</span>
          )}
        </div>

        {isInterview && (
          <div className="flex items-center gap-2 mb-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
        ...
        {isInterview && application.interviewTime && (
          <div className="mb-3 flex items-center gap-2 px-2.5 py-1.5 bg-primary-light/50 rounded-lg border border-primary/10">
            <Clock size={12} className="text-primary flex-shrink-0" />
            <span className="text-xs font-semibold text-slate-700">{application.interviewTime.replace('T', ' ')}</span>
            {countdown && (
              <span className={`text-xs font-bold ml-auto ${
                countdown === '进行中' ? 'text-red-500 animate-pulse' : 'text-primary'
              }`}>{countdown}</span>
            )}
          </div>
        )}
        ...
        {isInterview && (
          <div className="mb-3">
            <label className="flex items-center gap-2 cursor-pointer group" onClick={handleTogglePrepared}>
              {application.prepared ? (
                <CheckCircle2 size={14} className="text-primary flex-shrink-0" />
              ) : (
                <Circle size={14} className="text-gray-300 group-hover:text-gray-400 flex-shrink-0" />
              )}
              <span className={`text-xs font-medium ${application.prepared ? 'text-primary' : 'text-gray-400 group-hover:text-gray-500'}`}>
                {application.prepared ? '已准备就绪 ✓' : '已熟悉 JD / 已准备自我介绍'}
              </span>
            </label>
          </div>
        )}
        ...
        {application.notes && !isEditingNotes && (
          <div className="mb-4 px-2.5 py-1.5 bg-gray-50 rounded-lg">
